...
/*
  ...
  */
  ...
  // Simple stable string hash -> 32-bit unsigned (Same trick we used in Hangmandle!)
  ...
  // Hash the date string to get a pseudo-random, but globally synced index
  const salt = "factbookle_daily_salt_v1";
  const dailyIndex = hashStringToUint32(`${salt}:${todayStr}`) % countryFiles.length;
  let currentTargetFile = countryFiles[dailyIndex];
